...
export default class GoogleSearch {
    ...
    constructor ( { proxy, datasets, maxRetries = 10 } = {} ) {
        ...
        this.#agent = new Agent( { "keepAlive": true, "keepAliveMsecs": 300000, "proxy": this.#proxy } );
    }
    ...
    async search ( { keyword, target, maxResults = 100, location, randomCoordinates = false, minDistance, maxDistance, proxy, language } ) {
        ...
        if ( typeof location === "string" ) {
            ...
            if ( randomCoordinates ) {
                ...
                if ( !geolocation.data?.coordinates ) return result( [500, `Unable to get coordinates for location`] );

                coordinates = geolocation.data.coordinates;
            }
        }
        else {
            if ( maxDistance ) {
