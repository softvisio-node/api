...
export default class Anticaptcha extends CaptchaApi {
    ...
    async resolveImageCaptcha ( image, { signal, phrase, caseSensitive, numeric, calc, minLength, maxLength, comment, websiteUrl } = {} ) {
        ...
        return this.#getResult( res.data.taskId, "ImageToTextTask", signal );
    }
    ...
    async resolveRecaptchaV2 ( websiteUrl, websiteKey, { signal } = {} ) {
        ...
        res = await this.#getResult( res.data.taskId, "recaptchaV2TaskProxyless", signal );
    ...
    async #request ( method, path, body = {}, signal ) {
        ...
        while ( 1 ) {
            ...
            // captcha not ready
            if ( data.errorCode === "CAPCHA_NOT_READY" ) {
    ...
    async #getResult ( id, type, signal ) {
        while ( 1 ) {
            ...
            const res = await this.#request( "post", "getTaskResult", { "taskId": id }, signal );
            ...
            // if ( !res.ok ) return new Captcha( this, id, type, res, null, null );
