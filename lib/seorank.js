...
export default class SeoRank extends ThreadsPool {
    ...
    async getMoz ( domain ) {
        return this.addThread( this.#thread.bind( this ), `https://seo-rank.my-addr.com/api2/moz/${this.#apiKey}/${domain}` );
    }

    // XXX up to 40 threads
    // XXX do not use this shit
    async getMajestic ( url ) {
        return this.addThread( this.#thread.bind( this ), `https://seo-rank.my-addr.com/api3/${this.#apiKey}/${url}` );
    }
